...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // 按照百分比宽度来修改pizza尺寸，简单方便快捷，不在循环中使用offsetwidth，防止出现FSL。
...
function updatePositions() {
  ...
  bodyScrollTop = document.body.scrollTop / 1250; // 将scrollTOP放置在循环外，防止出现FSL
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  items = document.querySelectorAll('.mover'); // DOM加载完毕就计算背景pizza数量。
  updatePositions();
});
